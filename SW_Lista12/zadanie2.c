...
volatile float target_Temperature  = 0;
...
ISR(TIMER2_COMPA_vect)
{
    ...
    {
        value = readVoltage();
        float V_IN = (value * 1.1) / 1024;
        float ReadTemperature = (V_IN - V_0) / TemperatureCoefficient;
        printf("\r%6u %7u %7u %7d     %6f     %6f",value, target, OCR1A, v, ReadTemperature, target_Temperature);
        i = 0;
    }
}
...
ISR(TIMER0_OVF_vect)
{
    ...
    if(i < TIME_INTERVAL)
        ...
        int16_t inputValue = pid_Controller(target, value, &pidData);
...
int main()
{
    ...
    printf("odczyt     cel     PWM     PID    RTemp          TTemp\r\n");
    adc_init();
    sei();
    DDRB |= _BV(PB1);
    while(1) {
        if(scanf("%f",&target_Temperature))
        {
            target = (uint16_t)((((target_Temperature + V_0)*TemperatureCoefficient)*1024) + 300);
